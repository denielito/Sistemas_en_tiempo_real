...
static esp_err_t http_server_index_html_handler(httpd_req_t *req) {
    ...
    char time_buffer[9];  // Suficiente para "HH:MM:SS\0"
    get_current_time(time_buffer, sizeof(time_buffer));

    httpd_resp_sendstr(req, "<html><body><h1>Hora actual: ");
    httpd_resp_sendstr(req, time_buffer);
    httpd_resp_sendstr(req, "</h1></body></html>"); */

	httpd_resp_set_type(req, "text/html");

    // En el siguiente código HTML, se utiliza JavaScript para actualizar la hora cada segundo.
    const char *html_response =
        ...
        "</head>"
        "<body>"
        "<h1>Hora actual: <span id='clock'></span></h1>"
        "</body>"
        "</html>";
