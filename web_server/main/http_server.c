...
esp_err_t http_server_toggle_led_handler(httpd_req_t *req);
...
void initialize_sntp(void) // Inicialización del SNTP
{
	...
	esp_sntp_setoperatingmode(SNTP_OPMODE_POLL); /*Se indica que va a trabajar en modo SNTP_OPMODE_POLL. significa que SNTP
	...
	esp_sntp_setservername(0, "pool.ntp.org");						   // Se utiliza para configurar el nombre del servidor NTP que SNTP utilizará para sincronizar la hora
	...
	esp_sntp_init();												   // Se inicializa el cliente o servicio SNTP
}
...
esp_err_t root_post_handler(httpd_req_t *req)
{
    ...
    if (buf_len > 1) {
        ...
        if (httpd_req_get_url_query_str(req, buf, buf_len) == ESP_OK) {
            if (httpd_query_key_value(buf, "color", param, sizeof(param)) == ESP_OK) {
                // param ahora tiene el color del LED (r, g, o b)
                if (strcmp(param, "r") == 0) {
                    toggle_led(LEDR);
    ...
    // Respondemos con un JSON indicando el estado actual de los LEDs
    httpd_resp_set_type(req, "application/json");
    httpd_resp_send(req, "{\"led_r\":\"ON\",\"led_g\":\"OFF\",\"led_b\":\"ON\"}", -1);
...
*/
		...
		httpd_register_uri_handler(http_server_handle, &toggle_led_uri);
